 ...
 *
 * Registers nine read-only, loopback-only data endpoints on the web server:
		...
		if (scanPersisted && persistence !== void 0) {
			...
			if (snapshots !== null) {
				for (const { header: meta, revision } of snapshots) {
					...
					if (changed) {
						try {
							if (await foldPersistedSession(ctx, persistence, id, state, estimateCost, revision)) stateChanged = true;
						} catch (error) {
