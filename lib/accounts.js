...
// Command Code's account endpoints currently live under the native alpha
// namespace. They are intentionally isolated here so a future upstream API
// can be added without changing the account protocol or credential seam.
const COMMAND_CODE_CREDITS_PATH = "/alpha/billing/credits";
const COMMAND_CODE_SUBSCRIPTION_PATH = "/alpha/billing/subscriptions";
const COMMAND_CODE_USAGE_PATH = "/alpha/usage/summary";
...
function adapterMode(adapter, monitor) {
	...
	if (adapter === "command-code") return "subscription";
...
/** Return the untransformed value for a mapping so callers can distinguish missing from malformed data. */
function mappedSource(root, mapping) {
	if (mapping === void 0 || mapping === null) return void 0;
	if (typeof mapping === "string") return jsonPointer(root, mapping);
	if (typeof mapping === "object" && typeof mapping.pointer === "string") return jsonPointer(root, mapping.pointer);
	return void 0;
}
...
function commandCodeCredits(body) {
	const value = body?.credits ?? body?.data?.credits ?? body?.data ?? body;
	return value !== null && typeof value === "object" && !Array.isArray(value) ? value : null;
}

function commandCodeWindow(kind, raw, fallbackResetAt = null) {
	if (raw === null || typeof raw !== "object" || Array.isArray(raw)) return null;
	const used = numberOrNull(raw.used);
	const cap = numberOrNull(raw.cap ?? raw.limit);
	if (used === null || cap === null || cap <= 0) return null;
	const usedPercent = round1(Math.max(0, Math.min(100, used / cap * 100)));
	// A non-positive reset timestamp marks a window that has not started yet (the
	// 5-hour window opens with the first request of the period). Reading it as an
	// epoch would show a permanent "reset due" state.
	const rawReset = [raw.resetAt, raw.reset_at, raw.resetsAt, fallbackResetAt]
		.find((value) => value !== void 0 && value !== null && value !== "" && (typeof value !== "number" || value > 0));
	const resetsAt = rawReset === void 0 ? null : toIso(rawReset);
	return {
		kind,
		usedPercent,
		remainingPercent: round1(100 - usedPercent),
		started: resetsAt !== null,
...
/**
 * The alpha API never states the monthly allowance: /alpha/billing/credits
 * reports what is left in the current billing period and /alpha/usage/summary
 * reports what that period has spent. Their sum is the effective allowance,
 * marked `derived` so callers can present it as an estimate; without the usage
 * summary the row is omitted instead of invented.
 */
function commandCodeMonthlyWindow(periodSpent, remainingMonthly, resetsAt) {
	if (periodSpent === null || remainingMonthly === null || periodSpent < 0 || remainingMonthly < 0) return null;
	const window = commandCodeWindow("monthly", { used: periodSpent, cap: round1(periodSpent + remainingMonthly), resetAt: resetsAt });
	return window === null ? null : { ...window, derived: true };
}


function commandCodeSubscription(body) {
	const value = body?.data ?? body?.subscription ?? body;
	return value !== null && typeof value === "object" && !Array.isArray(value) ? value : null;
}

/**
 * Query Command Code's native account/rolling-window API.
 *
 * The provider API documentation currently covers model inference routes, but
 * the CLI's account surface exposes the credit and window data needed by this
 * panel. Keep this adapter opt-in and fail soft when the alpha surface changes.
 */
async function queryCommandCode(spec, credential, deps, now) {
	const headers = {
		authorization: `Bearer ${credential}`,
		"x-api-key": credential,
		accept: "application/json"
	};
	const body = await requestJson(new URL(COMMAND_CODE_CREDITS_PATH, spec.baseURL).href, { headers }, deps);
	const credits = commandCodeCredits(body);
	if (credits === null) throw statusError("invalid-response", "Command Code credits response is invalid");
	const monthly = numberOrNull(credits.monthlyCredits ?? credits.monthly_credits);
	const purchased = numberOrNull(credits.purchasedCredits ?? credits.purchased_credits);
	const free = numberOrNull(credits.freeCredits ?? credits.free_credits);
	const creditParts = [monthly, purchased, free].filter((value) => value !== null);
	if (creditParts.length === 0) throw statusError("invalid-response", "Command Code credits response is missing credit fields");

	const limits = body?.windowLimits ?? body?.window_limits ?? credits.windowLimits ?? credits.window_limits ?? {};
	let subscription = null;
	try {
		subscription = commandCodeSubscription(await requestJson(new URL(COMMAND_CODE_SUBSCRIPTION_PATH, spec.baseURL).href, { headers }, deps));
	} catch {
		// Credits and windows remain useful if the optional plan endpoint is
		// absent, temporarily unavailable, or changed independently.
	}
	let usageSummary = null;
	try {
		usageSummary = await requestJson(new URL(COMMAND_CODE_USAGE_PATH, spec.baseURL).href, { headers }, deps);
	} catch {
		// The billing-period spend only feeds the monthly row; without it
		// that row is omitted instead of being invented.
	}

	const plan = nonEmptyString(subscription?.planId ?? subscription?.plan_id ?? subscription?.planName ?? subscription?.plan);
	const expiresAt = toIso(subscription?.currentPeriodEnd ?? subscription?.current_period_end ?? subscription?.expiresAt);
	const periodSpent = numberOrNull(usageSummary?.totalCredits ?? usageSummary?.total_credits ?? usageSummary?.totalCost ?? usageSummary?.total_cost);
	const windows = [
		commandCodeWindow("session", limits.fiveHour ?? limits.five_hour),
		commandCodeWindow("weekly", limits.weekly),
		commandCodeMonthlyWindow(periodSpent, monthly, expiresAt)
	].filter(Boolean);

	const remaining = creditParts.reduce((sum, value) => sum + value, 0);
	const balance = {
		remaining,
		currency: "USD",
		unlimited: false,
	...
	return {
		...
		balance,
		windows,
		alert: windows.length > 0 ? subscriptionAlert(windows) : balanceAlert(balance, spec.monitor.warning),
		...(subscription?.status === void 0 ? {} : { subscriptionStatus: subscription.status })
	};
}
...
function customBalance(spec, body, now) {
	...
	const remainingValue = mapped(root, extract.remaining);
	const remainingSource = mappedSource(root, extract.remaining);
	const usedRaw = numberOrNull(mapped(root, extract.used));
	const totalRaw = numberOrNull(mapped(root, extract.total));
	let remainingRaw;
	if (remainingSource !== void 0 && remainingSource !== null) {
		remainingRaw = numberOrNull(remainingValue);
		if (remainingRaw === null) throw statusError("invalid-response", "custom response remaining is not numeric");
	} else if (usedRaw !== null && totalRaw !== null) {
		remainingRaw = Math.max(0, totalRaw - usedRaw);
	} else {
		throw statusError("invalid-response", "custom response is missing a numeric balance");
	}
	const balance = {
...
export async function queryAccount(spec, credentials, deps = {}) {
	...
	try {
		...
		if (spec.adapter === "command-code") return finish(await queryCommandCode(spec, credential, safeDeps, now));
