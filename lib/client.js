...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function UsageStatsPanel({ wide, t }) {
			...
			const selectProviderChoice = react.useCallback((providerId) => {
		...
		/**
		 * Window footer for Command Code rows: the 5-hour window reports `resetAt: 0`
		 * until the first request starts it, so it explains itself instead of showing a
		 * bogus "reset due" state.
		 */
		function commandCodeResetLabel(window, translate) {
			const countdown = resetLabel(window?.resetsAt, translate);
			if (countdown !== "") return countdown;
			return window?.kind === "session" && window?.started === false ? translate("commandCode.notStarted") : "";
		}

		/** Command Code reports credit pools plus 5-hour, weekly and billing-period windows. */
		function CommandCodeContent({ provider, translate }) {
			const balance = provider?.balance ?? null;
			const windows = Array.isArray(provider?.windows) ? provider.windows : [];
			const rows = [
				{ value: balance?.breakdown?.monthly, label: translate("commandCode.monthly") },
				{ value: balance?.breakdown?.purchased, label: translate("commandCode.purchased") },
				{ value: balance?.breakdown?.free, label: translate("commandCode.free") }
			].filter((row) => row.value !== null && row.value !== void 0);
			return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, {
				children: [
					balance === null ? react_jsx_runtime.jsx("p", { className: S.quotaEmpty, children: translate("commandCode.noCredits") }) : react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, {
						children: [
							react_jsx_runtime.jsxs("div", {
								className: S.balanceMain,
								children: [
									react_jsx_runtime.jsx("span", { className: S.balanceAmount, children: fmtCurrency(balance.remaining, balance.currency ?? "USD") }),
									react_jsx_runtime.jsx("span", { className: S.accountPlan, children: translate("commandCode.available") })
								]
							}),
							rows.length > 0 ? react_jsx_runtime.jsx("div", {
								...
								children: rows.map((row) => react_jsx_runtime.jsxs("div", {
									...
									children: [
										react_jsx_runtime.jsx("span", { children: row.label }),
										react_jsx_runtime.jsx("span", { children: fmtCurrency(row.value, balance.currency ?? "USD") })
									]
								}, row.label))
							}) : null
						]
					}),
					windows.length > 0 ? react_jsx_runtime.jsx("div", {
						className: S.quotaList,
						children: windows.map((window) => {
							const used = Math.max(0, Math.min(100, Number(window.usedPercent) || 0));
							const value = typeof window.usedCredits === "number" && typeof window.capCredits === "number"
								? `${fmtCurrency(window.usedCredits, "USD")} / ${fmtCurrency(window.capCredits, "USD")} · ${used.toFixed(used % 1 === 0 ? 0 : 1)}%`
								: translate("subscription.used", { value: used.toFixed(used % 1 === 0 ? 0 : 1) });
							return react_jsx_runtime.jsxs("div", {
								className: S.quotaRow,
								children: [
									react_jsx_runtime.jsxs("div", {
									...
									children: [
										react_jsx_runtime.jsx("span", { className: S.quotaLabel, children: quotaLabel(window.kind, translate) }),
										react_jsx_runtime.jsx("span", { className: S.quotaReset, children: commandCodeResetLabel(window, translate) }),
										react_jsx_runtime.jsx("span", { className: S.quotaValue, children: value })
									]
								}),
								react_jsx_runtime.jsx("div", {
									className: S.quotaTrack,
		...
		function ProviderAccountCard({ provider, account, accountLoading, accountError, translate, onRetry }) {
			...
			const commandCodeMode = account?.adapter === "command-code" || provider.adapter === "command-code";
			const subscriptionMode = mode === "subscription" && !commandCodeMode;
			...
			return react_jsx_runtime.jsxs("article", {
				...
				commandCodeMode
					? accountError !== null ? react_jsx_runtime.jsxs("div", {
						...
						? react_jsx_runtime.jsx("p", { className: S.quotaEmpty, children: translate("commandCode.loading") })
						: react_jsx_runtime.jsx(CommandCodeContent, { provider: account ?? { status: "unavailable", balance: null, windows: [] }, translate })
				: subscriptionMode
						? accountError !== null ? react_jsx_runtime.jsxs("div", {
							className: S.error,
							children: [
		...
		const zh = {
			...
			"commandCode.available": "可用 credits",
			"commandCode.monthly": "月度 credits",
			"commandCode.purchased": "购买 credits",
			"commandCode.free": "免费 credits",
			"commandCode.loading": "正在查询 Command Code 额度…",
			"commandCode.noCredits": "Command Code 没有返回可识别的 credits。",
	"commandCode.notStarted": "首次调用后开始计时",
		...
		const en = {
			...
			"commandCode.available": "Available credits",
			"commandCode.monthly": "Monthly credits",
			"commandCode.purchased": "Purchased credits",
			"commandCode.free": "Free credits",
			"commandCode.loading": "Fetching Command Code credits…",
			"commandCode.noCredits": "Command Code returned no recognizable credits.",
	"commandCode.notStarted": "Starts on the first request",
		...
		exports.commandCodeResetLabel = commandCodeResetLabel;
